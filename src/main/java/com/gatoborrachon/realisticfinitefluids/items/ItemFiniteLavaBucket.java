...
public class ItemFiniteLavaBucket extends ItemBucket  {

    private final Block containedBlock;

      public ItemFiniteLavaBucket(String name, Block containedBlock) {
        ...
        public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
            ...
            if (handler != null) {
                FluidStack fluid = new FluidStack(FluidRegistry.LAVA, Fluid.BUCKET_VOLUME);
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                    	...
                    	BlockFiniteFluid.setBlockState(worldIn, posIn, BlockFiniteFluid.setVolume(null, null, ModBlocks.FINITE_LAVA_FLOWING.getDefaultState(), BlockFiniteFluid.MAXIMUM_LEVEL));
		...
		public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand,
            ...
            if (stack.getItem() == ModItems.FINITE_LAVA_BUCKET) {
                ...
                if (worldIn.isAirBlock(targetPos) || targetBlock instanceof BlockFiniteFluid) {
                    if (!worldIn.isRemote) {
                    	if (targetBlock instanceof BlockFiniteFluid && worldIn.getBlockState(targetPos).getMaterial() == Material.LAVA) {
                    		...
                    		if (BlockFiniteFluid.getConceptualVolume(worldIn, targetPos, targetBlockState) == BlockFiniteFluid.MAXIMUM_CONCEPTUAL_LEVEL) worldIn.setBlockState(targetPos.up(), ModBlocks.FINITE_LAVA_FLOWING.getDefaultState());
                    		else
                    	...
                    	} else if (targetBlock instanceof BlockFiniteFluid && worldIn.getBlockState(targetPos).getMaterial() == Material.WATER) {
                        	...
                        	BlockFiniteFluid.setBlockState(worldIn, targetPos, BlockFiniteFluid.setVolume(null, null, ModBlocks.FINITE_LAVA_FLOWING.getDefaultState(), BlockFiniteFluid.MAXIMUM_LEVEL));
                    ...
                    worldIn.playSound(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D,
                    		SoundEvents.ITEM_BUCKET_EMPTY_LAVA, SoundCategory.BLOCKS, 1.0F, 1.0F, false);
        ...
        public static void distributeFluidEqually(World world, BlockPos pos, int incomingLevel) {
            ...
            if (centerState.getBlock() instanceof BlockFiniteFluid && centerState.getMaterial() == Material.LAVA) {
                ...
                int toAdd = Math.min(BlockFiniteFluid.MAXIMUM_CONCEPTUAL_LEVEL - currentLevel, remaining); //remaining es el maximo, 16 LEVELs conceptuales
            	...
            	BlockFiniteFluid.setBlockState(world, pos, BlockFiniteFluid.setConceptualVolume(null, null, ModBlocks.FINITE_LAVA_FLOWING.getDefaultState(), remaining));
            }
            ...
            for (BlockPos p : laterals) {
                ...
                if (s.getBlock() instanceof BlockFiniteFluid && s.getMaterial() == Material.LAVA && BlockFiniteFluid.getConceptualVolume(world, p, s) < BlockFiniteFluid.MAXIMUM_CONCEPTUAL_LEVEL) {
            ...
            remaining = FiniteFluidLogic.FluidWorldInteraction.distributeEqually(world, lateralTargets, remaining, 1); //1 --> FluidType of lava

            // 5. Si sigue sobrando, distribuye a diagonales
            remaining = FiniteFluidLogic.FluidWorldInteraction.distributeEqually(world, diagonalTargets, remaining, 1); //1 --> FluidType of lava
            ...
            if (remaining > BlockFiniteFluid.MINIMUM_LEVEL && remaining <= BlockFiniteFluid.MAXIMUM_CONCEPTUAL_LEVEL) { //ChatGPT dijo que <16 --> <=16
                ...
                if (world.isAirBlock(above)) {
                	...
                	BlockFiniteFluid.setBlockState(world, above, BlockFiniteFluid.setConceptualVolume(null, null, ModBlocks.FINITE_LAVA_FLOWING.getDefaultState(), remaining));
        ...
        public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable NBTTagCompound nbt) {
            return new FluidBucketWrapper(stack) {
                ...
                public FluidStack drain(FluidStack resource, boolean doDrain) {
                    if (resource != null && resource.getFluid() == FluidRegistry.LAVA) {
                        if (doDrain) return new FluidStack(FluidRegistry.LAVA, Fluid.BUCKET_VOLUME);
                        return resource;
                    }
                    return null;
                }
