...
public class RenderNewFluidsClassic {
	...
	public List<BakedQuad> renderBlockNewGasClassic(IBlockState state, 
		...
		if (!isStill) {
			...
			if (hasFlow) angle = (float) Math.atan2(fx, -fz); // misma convenci�n que usabas
		}
		...
		if (renderTop && (sideRequested == null || sideRequested == EnumFacing.UP) 
			...
			if (isStill) {
				// top quad con UVs sin rotaci�n ni offsets
				...
				// HAY FLUJO: usa m�todo tipo vanilla para calcular las UVs por v�rtice
				// No escalamos `baseU0..baseV1`. En su lugar calculamos UVs por v�rtice en "pixel coords"
				...
				// Nota: el orden de v�rtices debe coincidir con el que espera el renderer (igual que tu c�digo original)
			...
			return quads; // Si solo quer�an UP, devuelve aqu�
		...
		for (EnumFacing face : sides) {
			if (sideRequested != null && face != sideRequested) continue; // si piden una cara espec�fica, las demas las descartamos
	...
	//Esta funcion es la que se encarga de a�adirle los respectivos parametros de posicion, color, coordenada UV, etc a cada vertice unico (4 por cara) que vamos a renderizar
